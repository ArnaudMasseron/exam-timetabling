...
XLSX.openxlsx(exam_list_file) do workbook
    ...
    for row in XLSX.eachtablerow(sheet)
        ...
        @assert(
            prod([students[i][1] == students[i+1][1] for i in 1:length(students)-1]),
        ...
        for e in group_examiner_acronyms
            push!(examiner_data, e => Dict{String, Any}(
...
XLSX.openxlsx(teacher_schedule_file) do workbook
    ...
    for row in XLSX.eachtablerow(sheet)
        ...
        if haskey(examiner_data, teacher_acronym)
            push!(examiner_data[teacher_acronym]["soft_obligations"], (DateTime(date, start_time), DateTime(date, end_time)))
        end
    end
end
...
dataset["students"] = Vector{Dict{String, Any}}([Dict{String, Any}() for i in 1:length(student_data)])
...
dataset["examiners"] = Vector{Dict{String, Any}}([Dict{String, Any}() for e in 1:length(examiner_data)])
for (key, value) in examiner_data
    dataset["examiners"][value["id"]] = Dict(
        "acronym" => key,
        "soft_unavailabilities" => Vector{Tuple{DateTime, DateTime}}(value["soft_obligations"]),
        "hard_unavailabilities" => Vector{Tuple{DateTime, DateTime}}(),
        "max_number_exams_per_day" => 24
    )
end

dataset["subjects"] = Vector{Dict{String, Any}}([Dict{String, Any}() for s in 1:length(subject_data)])
...
dataset["rooms"] = Vector{Dict{String, Any}}([Dict{String, Any}() for j in 1:length(room_data)])
...
dataset["groups"] = Vector{Dict{String, Any}}([Dict{String, Any}() for j in 1:length(group_set)])
...
for item in exam_set
    ...
    @assert(
        length(group_candidates) == 1,
        "One group has multiple ids in group_set"
    )
